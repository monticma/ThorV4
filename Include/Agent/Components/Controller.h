...
{
    int defaultPort = 0, commandPort = 0, messagePort = 0;
    std::string description;
};
struct SerialConnection
{
    int defaultBaud = 0, dataBits = 0, stopBits = 0;
    std::string parity;
};
...
{
    std::string identifier, library, driverConfig, description;
};
struct ControllerUnits
{
...
{
    double maxVelocity = 0.0, maxAcceleration = 0.0;
    std::string velocityUnit, accelerationUnit, description;
};
...
struct ControllerIoCapabilities
{
    ...
    std::string digitalVoltage, analogVoltage;
    int maxCurrentPerOutput = 0;
    std::string currentUnit, description;
};
