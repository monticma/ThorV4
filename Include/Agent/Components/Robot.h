...
struct DHParameter
{
    double a = 0.0, alpha = 0.0, d = 0.0;
    std::string theta, dVar, type, description;
};
struct RobotWorkspace
{
    double minReach = 0.0, maxReach = 0.0, minZ = 0.0, maxZ = 0.0;
};
struct RobotKinematics
{
    std::string type, coordinateSystem, visualizerType, pluginLibrary, pluginConfig;
    RobotWorkspace workspace;
    std::vector<DHParameter> dhParameters;
};
struct RobotCapabilities
{
    std::vector<std::string> primitives, features;
};
struct RobotPhysical
{
    double reach = 0.0, zTravel = 0.0, weight = 0.0;
    std::string weightUnit, mountType, environment, description;
};
struct RobotUnits
{
    std::string length, angle, linearVelocity, angularVelocity, linearAcceleration, angularAcceleration;
};
struct RobotVariant
{
    std::string model;
    double reach = 0.0, zTravel = 0.0;
    std::string description;
};
struct RobotMetadata
{
    std::vector<RobotVariant> variants;
